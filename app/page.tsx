  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center bg-graphite text-gold">
      ...
      <nav className="flex flex-wrap gap-4 mb-8">
        ...
        {role === 'OWNER' && <Link href="/admin" className="px-8 py-3 rounded bg-gold text-graphite font-extrabold text-lg shadow hover:bg-yellow-400 transition">Owner Panel</Link>}
        {role === 'CREATOR' && <Link href="/creator" className="px-8 py-3 rounded bg-gold text-graphite font-extrabold text-lg shadow hover:bg-yellow-400 transition">Creator Panel</Link>}
        {role === 'STAFF' && <Link href="/staff" className="px-8 py-3 rounded bg-gold text-graphite font-extrabold text-lg shadow hover:bg-yellow-400 transition">Staff Panel</Link>}
        {(role === 'USER' || role === 'PREMIUM' || role === 'GUEST') && <Link href="/dashboard" className="px-8 py-3 rounded bg-gold text-graphite font-extrabold text-lg shadow hover:bg-yellow-400 transition">Dashboard</Link>}
      </nav>
